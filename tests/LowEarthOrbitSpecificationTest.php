...
class LowEarthOrbitSpecificationTest extends TestCase
{
    public function testSpecificationIsSatisfied(): void
    {
        // ISS is in LEO at ~420 km altitude
        $tle = new Tle(
        ...
        $specification = new LowEarthOrbitTleSpecification();
        $this->assertTrue($specification->isSatisfiedBy($tle), 'ISS should satisfy LEO specification');
    }

    public function testSpecificationIsNotSatisfied(): void
    {
        // GOES-16 is a geostationary satellite (~35,786 km altitude)
        $tle = new Tle(
        ...
        $specification = new LowEarthOrbitTleSpecification();
        $this->assertFalse($specification->isSatisfiedBy($tle), 'Geostationary satellite should not satisfy LEO specification');
    }
}
